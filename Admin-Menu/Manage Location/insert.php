...
if(isset($_POST['submit'])){
	...
	$extension = substr($lpic,strlen($lpic)-4,strlen($lpic));
	...
	if(!in_array($extension,$allowed_extensions)){
		...
		$imgnewfile=md5($lpic).time().$extension; // Fixed variable name 

		move_uploaded_file($_FILES["lpic"]["tmp_name"],"../../Images/locationImages/".$imgnewfile);

		$query=mysqli_query($conn, "insert into tbllocation(title, location, rating, description, lpic, description2, price) value('$title','$location', '$rating', '$description','$imgnewfile', '$description2', '$price' )");
	...
	<body>
		<div class="l-form">
			<form  method="POST" enctype="multipart/form-data">
				...
				<div class="form-group">
					<input type="text" class="form-control" name="rating" placeholder="Rating" required="true">
				</div>
				
				<div class="form-group">
					<input type="text" class="form-control" name="price" placeholder="$700" required="true">
				</div>
				...
				<div class="form-group">
					...
					<textarea id="myTextarea" name="description2" rows="10" ></textarea>
				...
				<div class="text-center"><a href="managelocation.php">Back</a></div>
			</form>
