...
if(isset($_POST['submit'])){
	...
	$oldgalleryPic=$_POST['oldpic'];
	$oldgalleryPic="../../Images/galleryImages/"."/".$oldgalleryPic;

	$extension = substr($galleryPic, strrpos($galleryPic, '.'));
	...
	if(!in_array($extension,$allowed_extensions)){
		...
		$imgfile = $_FILES["galleryPic"]["tmp_name"];
		$imgnewfile = md5_file($imgfile) . time() . $extension;

		move_uploaded_file($_FILES["galleryPic"]["tmp_name"],"../../Images/galleryImages/".$imgnewfile);
  		
     	$query=mysqli_query($conn, "update tblgallery set galleryPic='$imgnewfile' where id='$uid' ");
    	if ($query) {
 
    		unlink($oldgalleryPic);
...
<html lang="en">
	...
	<body>
    	...
    	<form  method="POST" enctype="multipart/form-data">
 		<?php
			$eid=$_GET['userid'];
			$ret=mysqli_query($conn,"select * from tblgallery where id='$eid'");
			while ($row=mysqli_fetch_array($ret)) {
		?>
			<h2>Change</h2>
			<p class="hint-text">Display Pic</p>
			<input type="hidden" name="oldpic" value="<?php  echo $row['galleryPic'];?>">
			<div class="form-group">
			<img src="../../Images/galleryImages//<?php  echo $row['galleryPic'];?>" width="120" height="120">
		</div>
		...
		<div class="text-center"><a href="managegallery.php" class="back">Cancel</a></div>
		<?php 
		}?>
    	</form>
