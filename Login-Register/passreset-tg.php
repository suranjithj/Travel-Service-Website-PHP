...
if(isset($_POST['submit'])){
    ...
    if($password == $confirmpassword){
        ...
        $query=mysqli_query($conn, "UPDATE tourguide SET tg_password='$password' WHERE tg_email='$email' LIMIT 3");
        
        if ($query) {
        echo "<script>alert('You have successfully reset password ! Please Login');</script>";
        echo "<script type='text/javascript'> document.location ='tourguide-log-reg.php'; </script>";
        }
        else{
