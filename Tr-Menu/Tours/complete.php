...
if(isset($_POST['submit']))
  {
  	$eid=$_GET['editid'];
  	//Getting Post Values
	$trconfirm=$_POST['trconfirm'];


    //Query for data updation
     $query=mysqli_query($conn, "update booking set tr_confirm='$trconfirm' where bk_id='$eid'");
...
<html lang="en">
	...
	<body>
		<div class="update-form">
			...
			<?php
			$eid=$_GET['editid'];
			$ret=mysqli_query($conn,"select * from booking where bk_id='$eid'");
			while ($row=mysqli_fetch_array($ret)) {
			?>
				<h2>Update </h2>

				<div class="form-group">
                    <span>This Tour is Completed?</span>
					<input type="text" class="form-control" name="trconfirm" required >       	
				</div>

			<?php 
			}?>
				<div class="form-group">
					<button type="submit" class="btn btn-success btn-lg btn-block" name="submit" onclick="return confirm('Mark as Completed ?');">Update</button>
				</div>
